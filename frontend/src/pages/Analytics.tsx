...
const Analytics = () => {
  ...
  useEffect(() => {
    const loadAnalysis = async () => {
      // Check if we have a datasetId from the new flow
      if (location.state?.datasetId) {
        setLoading(true);
        try {
          const analysisResponse = await ApiService.getDatasetAnalysis(location.state.datasetId);
          const datasetResponse = await ApiService.getDataset(location.state.datasetId);
          ...
          <ChartContainer config={chartConfig} className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart data={chart.data}>
                ...
                <ChartTooltip content={<ChartTooltipContent />} />
                <Scatter dataKey={chart.yKey} fill="hsl(var(--chart-1))" />
              </ScatterChart>
            </ResponsiveContainer>
          </ChartContainer>
        );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8 space-y-8">
                ...
                <div key={index} className="p-4 border border-border rounded-lg">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      ...
                      {col.missing > 0 && <span>{col.missing} missing</span>}
                    </div>
                  </div>
                  <div className="text-sm text-muted-foreground">
                    <span className="font-medium">Sample values:</span> {col.sample.slice(0, 3).join(', ')}
                    {col.sample.length > 3 && '...'}
                  </div>
                </div>
              ))}
        ...
        {/* Statistical Summary */}
        {Object.keys(analysis.summary.numerical).length > 0 && (
          <Card className="shadow-card">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
